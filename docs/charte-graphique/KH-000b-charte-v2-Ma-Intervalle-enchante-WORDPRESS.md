...
Le visuel de la slide 4 était fusionné dans l’image du PowerPoint. Trois fichiers séparés sont désormais fournis dans `04_illustrations_editoriales` afin que Claude dispose à la fois de la référence exacte et d’un véritable fond de production.

| Fichier | Usage |
|---|---|
| `hero_slide4_extraction_exacte_avec_logo.jpg` | Référence fidèle de la composition d’origine, avec logo et textes déjà incrustés |
| `hero_fond_propre_4x3.png` | Fond nettoyé proche du cadrage source, sans logo ni texte |
| `hero_fond_web_16x9.png` | **Fond recommandé pour WordPress**, avec zone sûre à gauche pour les éléments HTML |

Le fichier 16:9 doit être appliqué en arrière-plan CSS. Le logo, le cartouche japonais, la tagline bilingue et l’indication de navigation doivent être reconstruits comme éléments HTML/CSS distincts pour préserver la netteté, l’accessibilité, la traduction et le responsive. Le koinobori ne doit être coupé ni sur ordinateur ni sur mobile ; si `background-size: cover` produit un recadrage dommageable, Claude doit créer un cadrage mobile dédié.
