...
| I2 | `polylang.redirect_lang = false` | `options.polylang` | `/fr/` et `/en/` servent l'index du blog (B4 confirmé) | PC4, `docs/development/polylang-reglages-requis.md` |
...
| I4 | **Notifications actives sur les 8 formulaires** (`notifications_enabled_any = true`) alors que FluentSMTP 2.3.1 n'est pas configuré (constat 10/09) | `forms[*]`, `plugins` | Les envois partent par `mail()` PHP ou échouent silencieusement ; sur le clone, les 12 modèles sont désactivés | A9, PC7 |
| I5 | `blogname` **vide** | `options.blogname` | Titre du site absent (balise `<title>`, Schema Organization, emails) | PC4 |
| I6 | Aucune zone d'expédition hors France : « Rest of the world » sans méthode ; `woocommerce_ship_to_countries = ""` (tous pays) | `shipping_zones`, `options` | Un client UE/US peut commander sans méthode de livraison → blocage au checkout ou livraison gratuite implicite selon réglage WC | PC4 : zone UE (Colissimo Zone A + MR), USA « nous contacter » = restreindre `ship_to_countries` |
...
| K2 | `seopress_xml_sitemap_general_enable = null` : option absente → sitemaps SEOPress **non activés** sur cette base (KH-109 les avait activés en juin sur `wp320` ; base différente) | `options` |
...
6. K2 sitemaps SEOPress, K3 Complianz, K1 LiteSpeed (chacun = point de contrôle 4).
