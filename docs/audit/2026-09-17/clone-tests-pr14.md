...
| `/fr/` toujours 403 après ce retrait, `WWW-Authenticate` présent | HEAD depuis le serveur : `/kh2027-preflight.php` 401, `/index.php` 401, `/fr/` 403, `.css` 401 ; journal d'accès : `/fr/contact/` déjà 403 le 16/09 16:51 | Première hypothèse (expression `Require expr` du bloc `index.php`) : `RequireAll` simplifié en `Require valid-user` par Alain → toujours 403. **Cause réelle** : en contexte `.htaccess`, la réécriture `RewriteRule . /index.php` s'exécute après le contrôle d'accès ; pour `/fr/` l'autorisation est évaluée sur le chemin « fr », refusé par `<FilesMatch ".*"> Require all denied`. Correction appliquée **par Alain** : `<FilesMatch "\.(php|phtml|phar|sql|log|json|txt|md|bak|gz|zip|ini|env)$">` à la place de `".*"` ; le `Require valid-user` du bloc cPanel couvre tout le reste |
| Vérification après correction | `/` `/fr/` `/en/` `/fr/contact/` `/index.php` `.css` → **401** ; `wp-login.php` `xmlrpc.php` `wp-config.php` → **403** | Clone privé conservé. Résidu : `wp-admin/` répond 401 (accessible après authentification, l'expression d'Astra l'excluait) ; acceptable pour une revue privée, à garder en tête. `.htaccess` final : SHA-256 `2b509380…b3c4`, copie `web-control/htaccess-after-fix-20260917.txt` |
...
| 6 HTTP `/fr/` `/en/` | **PASS** (après correction `.htaccess`) | HEAD serveur : 401 sans `Location` sur les deux ; après authentification basique saisie par Alain dans l'onglet : gabarit éditorial rendu (`id="kh-home-title"` présent), h1 « Des carpes de vent originales, signées BCDG » / « Original wind carps, signed by BCDG », header `kh-header is-ready` (JS sans exception), 5 rubriques ×2 (desktop + dialogue mobile) = Boutique · Lifestyle · L'Atelier · Professionnels · Contact / Shop · Lifestyle · The House · For Professionals · Contact, aucune icône neutre, sélecteur FR/EN, 4 liens légaux EN vers `/en/legal-notice/` `/en/terms-and-conditions/` `/en/privacy-policy/` `/en/cookie-policy/`, groupes footer « The house · Information · For professionals ». Observation hors périmètre : `<title>` de `/en/` = « Koinobori originaux signés BCDG » (non traduit, à traiter dans le lot S) |
