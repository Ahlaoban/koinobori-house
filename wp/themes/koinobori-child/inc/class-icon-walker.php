...
class Koinobori_Icon_Walker extends Walker_Nav_Menu {
	const KEYS = array( 'shop', 'lifestyle', 'house', 'professionals', 'contact' );

	/** Page slugs (FR and EN) that map to an approved icon when no icon-* class is set. */
	const SLUG_KEYS = array(
		'boutique' => 'shop', 'shop' => 'shop',
		'lifestyle' => 'lifestyle', 'lifestyle-koi' => 'lifestyle',
		'atelier' => 'house', 'the-house' => 'house',
		'professionnels' => 'professionals', 'professionals' => 'professionals',
	...
	public static function icon( $key ) {
		...
		return '<svg viewBox="0 0 24 24" width="28" height="28" aria-hidden="true" focusable="false">' . ( $icons[ $key ] ?? $icons['link'] ) . '</svg>';
	}

	/** icon-* class first, then the linked page slug or the shop page, else a neutral link icon. */
	public static function key_for( $item ) {
		foreach ( self::KEYS as $candidate ) {
			if ( in_array( 'icon-' . $candidate, (array) $item->classes, true ) ) { return $candidate; }
		}
		$object_id = (int) ( $item->object_id ?? 0 );
		if ( $object_id && function_exists( 'wc_get_page_id' ) && $object_id === (int) wc_get_page_id( 'shop' ) ) { return 'shop'; }
		if ( $object_id && 'page' === ( $item->object ?? '' ) ) {
			$slug = (string) get_post_field( 'post_name', $object_id );
			if ( isset( self::SLUG_KEYS[ $slug ] ) ) { return self::SLUG_KEYS[ $slug ]; }
		}
		return 'link';
	}
